"""
Librerías logica.py y otras funciones necesarias para el proyecto
"""
...
class Descriptor :
    ...
    def P(self, lista_valores):
        ...
        return chr(self.chrInit + int(codigo))

    def codifica(self, lista_valores):
        self.check_lista_valores(lista_valores)
        cod = int(lista_valores[0])  # Convert the first value to an integer
        n_columnas = 1

        for i in range(0, len(lista_valores) - 1):
            n_columnas = n_columnas * self.args_lista[i]
            cod = n_columnas * int(lista_valores[i + 1]) + cod  # Convert the other values to integers
...
def visualizar_regla(self, regla_result):
    for formula in regla_result.formulas:
        print(self.visualizar_formula(formula, self.OenP))
        print(" ")
